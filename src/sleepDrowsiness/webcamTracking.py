...
ALERT_REPEAT_SECONDS     = 1   # Delay between each beep repetition (longer = less aggressive)
...
class setupModel:
    ...
    def _generate_soft_chime(self) -> np.ndarray:
        ...
        envelope[release_start:] = np.linspace(1, 0, n_samples - release_start)  # long fade out
        ...
        while webcam.isOpened():
            ...
            if self.RES is not None:
                ...
                if len(y) > 0:
                    ...
                    # Probability < 0.5 → Drowsy, >= 0.5 → Awake
                    instant_label = "Drowsy" if modelOutput < 0.5 else "Awake"
            ...
            if self.alert_active and (now - self.alert_last_played) >= ALERT_REPEAT_SECONDS:
            ...
            if self.drowsy_since is not None:
                ...
                cv2.rectangle(frame, (25, 110), (25 + bar_width, 130), (0, 100, 255), -1)
                cv2.putText(
                    frame,
